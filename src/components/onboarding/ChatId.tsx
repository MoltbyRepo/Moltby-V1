...
import { ArrowRight, MessageSquare, Hash, AlertCircle, Loader2 } from "lucide-react";

interface StepProps {
  onNext: (value: string) => void;
  token?: string; // We need the token to validate the chat ID
}

export function ChatId({ onNext, token }: StepProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // If we have a token, we can validate the chat ID
      if (token) {
        const response = await fetch("/api/bot/validate-chatid", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ token, chatId: value.trim() }),
        });
        ...
        if (data.valid) {
          ...
          setError(data.error || "Invalid Chat ID or bot hasn't started a conversation with this user.");
        }
      } else {
        // Fallback if token is missing (shouldn't happen in normal flow)
        onNext(value.trim());
      }
    } catch (err) {
      setError("Failed to validate Chat ID. Please check your connection.");
    } finally {
    ...
    >
      <div className="bg-[#0f172a]/90 border border-blue-900/30 p-8 rounded-2xl max-w-md w-full shadow-2xl shadow-black/50 ring-1 ring-blue-500/10 backdrop-blur-xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-semibold text-white">Chat ID</h2>
            <p className="text-slate-400 text-sm">Enter your Telegram Chat ID for notifications.</p>
          </div>
        </div>
